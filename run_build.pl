...
my (
	...
	$use_installcheck_parallel, $fail_on_warning
  )
  = @PGBuild::conf{
	qw(build_root target animal aux_path trigger_exclude
	  ...
	  use_valgrind valgrind_options use_installcheck_parallel fail_on_warning)
  };
...
our $build_version=get_pg_version(($use_vpath?"pgsql":$pgsql). "/configure.in");

# Put version into config for modules
$PGBuild::conf{build_version} = $build_version;
